...
class ElasticsearchHandler(logging.Handler):
    ...
    def __init__(
        ...
        if username and password:
            self.connection_config.update({"http_auth": (username, password)})
    ...
    def _get_es_client(self):
        ...
        if self.es_client is None:
            try:
                ...
                raise ImportError(
                    "elasticsearch package is required for ElasticsearchHandler"
                )
...
class MemoryHandler(logging.handlers.MemoryHandler):
    ...
    def __init__(
        ...
        # 添加统计信息
        self.stats = {"records_buffered": 0, "records_flushed": 0, "flush_count": 0}
