# PRD: AI 知识管理系统（含动态知识图谱模块）

## 📋 文档信息

| 项目 | 内容 |
|------|------|
| **产品名称** | AI KnowledgeOS（知识管理系统） |
| **版本** | v1.1 |
...
构建一套 AI 驱动的**知识生命周期操作系统**，协助个人/团队完成“知识收集 → 内化消化 → 提取应用 → 链接沉淀”全流程。动态知识图谱是其中一个可视化模块，除此之外系统还要：

1. **智能采集** - 自动从多源内容（对话、文档、网页、邮件）抓取知识，并完成结构化录入
2. **深度内化** - 通过摘要、知识卡片、观点对比等方式，让内容可消费、可重组
3. **敏捷提取** - 在不同任务场景里快速召回知识（问答、模板、工作流）
4. **自演化链接** - 让知识之间形成可追踪的语义网络，支持洞察和知识二次创造
...
- **是什么**:
  - ✅ 一个覆盖知识 Capture / Internalize / Retrieve / Link 全链路的 AI 系统
  - ✅ 可以插拔模块（知识采集、卡片化、图谱、问答、报告生成）的知识操作平台
  - ✅ 面向知识工作者的“第二大脑”操作系统，动态知识图谱是可视化/链接层的核心模块

### 1.3 知识生命周期覆盖能力

| 环节 | 目标 | 关键能力 |
|------|------|----------|
| 采集 Capture | 低摩擦录入多源内容 | 多源抓取、自动分发、质量评估 |
| 内化 Internalize | 把信息转成可用知识 | 摘要、知识卡片、观点对比、个人化版本 |
| 提取 Retrieve | 在任务中迅速调用 | 场景化问答、Playbook、模板化交付 |
| 链接 Link | 形成演化中的知识网络 | 动态知识图谱、跨主题关联、洞察推送 |

> 本 PRD 聚焦“链接 Link”模块，但所有设计需与其它环节接口一致，确保知识在系统内可流动。

### 1.3 成功指标 (KPI)

| 指标类型 | 指标名称 | 目标值 | 衡量周期 |
|---------|---------|--------|---------|
| **采集** | 自动知识抓取覆盖率 | 70%+ 主题拥有自动输入 | 周 |
| **内化** | AI 生成知识卡片采用率 | 60%+ | 周 |
| **提取** | RAG 查询命中率 | >85% 相关回答 | 周 |
| **链接** | 用户发现新关联率 | 30%+ | 月 |
...
> **模块分层总览**  
> 1. **知识采集（Capture Hub）**：多源接入、自动解析、任务分发  
> 2. **知识内化（Insight Forge）**：结构化摘要、知识卡片、观点对比、版本化  
> 3. **知识提取（Action Engine）**：RAG 问答、任务模板、工作流调用  
> 4. **知识链接（Graph Studio）**：动态知识图谱、跨主题关联、洞察推送  
>
> 本文档详细描述第 4 层“Graph Studio”，并补充与其他层的接口需求，确保体验一致。
...
| 上游输入 | Graph Studio 接口需求 | 下游输出 |
|----------|-----------------------|----------|
| Capture Hub（文档 / 对话 / 邮件） | 提供结构化概念候选（concept + confidence + source） | Graph Studio 负责聚合、去重、落库 |
| Insight Forge（知识卡片） | 提供主题摘要、观点摘要，作为节点属性 | 图谱反哺 Insight Forge，推送“缺失片段”待内化 |
| Action Engine（RAG / 工作流） | 需要可引用的节点 ID / 关联路径 | 图谱提供“回答引用链路”“跨主题跳转” |

> 当其它模块尚未实现时，Graph Studio 需提供临时 API（/graph/intake、/graph/export）以便接入后续能力。
