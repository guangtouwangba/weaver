...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        # 学习分析字段
        sa.Column(
            "core_concepts_discovered", sa.INTEGER(), nullable=False, server_default="0"
        ),
        sa.Column(
            "concept_relationships", sa.INTEGER(), nullable=False, server_default="0"
        ),
        sa.Column(
            "missing_materials_count", sa.INTEGER(), nullable=False, server_default="0"
        ),
        # 关联字段
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["topic_id"],
            ["topics.id"],
            name="fk_topic_tags_topic_id",
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["tag_id"], ["tags.id"], name="fk_topic_tags_tag_id", ondelete="CASCADE"
        ),
    ...
    op.create_table(
        ...
        # 解析状态
        sa.Column(
            "is_parsed", sa.Boolean(), nullable=False, server_default=sa.text("false")
        ),
        sa.Column(
            "parse_status",
            parse_status_enum,
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["topic_id"],
            ["topics.id"],
            name="fk_conversations_topic_id",
            ondelete="SET NULL",
        ),
    )
    ...
    op.create_index(
        "idx_topic_resources_resource_type", "topic_resources", ["resource_type"]
    )
    op.create_index(
        "idx_topic_resources_parse_status", "topic_resources", ["parse_status"]
    )
    op.create_index(
        "idx_topic_resources_uploaded_at", "topic_resources", ["uploaded_at"]
    )
    op.create_index("idx_topic_resources_is_deleted", "topic_resources", ["is_deleted"])

    # GIN索引用于JSONB字段快速查询 (注意：使用jsonb_ops操作符类)
    op.execute(
        "CREATE INDEX idx_topic_resources_metadata_gin ON topic_resources USING gin (metadata jsonb_ops);"
    )
    op.execute(
        "CREATE INDEX idx_conversations_conversation_data_gin ON conversations USING gin (conversation_data jsonb_ops);"
    )

    op.create_index("idx_conversations_topic_id", "conversations", ["topic_id"])
    op.create_index("idx_conversations_created_at", "conversations", ["created_at"])
    op.create_index(
        "idx_conversations_last_message_at", "conversations", ["last_message_at"]
    )
