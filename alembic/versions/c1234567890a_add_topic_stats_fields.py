...
def upgrade() -> None:
    ...
    if not result.fetchone():
        op.add_column('topics', sa.Column('total_resources', sa.Integer(), nullable=True, server_default='0'))
    ...
    if not result.fetchone():
        op.add_column('topics', sa.Column('total_conversations', sa.Integer(), nullable=True, server_default='0'))
    ...
    if not result.fetchone():
        op.add_column('topics', sa.Column('core_concepts_discovered', sa.Integer(), nullable=True, server_default='0'))
    ...
    if not result.fetchone():
        op.add_column('topics', sa.Column('concept_relationships', sa.Integer(), nullable=True, server_default='0'))
    ...
    if not result.fetchone():
        op.add_column('topics', sa.Column('missing_materials_count', sa.Integer(), nullable=True, server_default='0'))
    ...
    if not result.fetchone():
        op.add_column('topics', sa.Column('last_accessed_at', sa.TIMESTAMP(timezone=True), nullable=True, server_default=sa.text('CURRENT_TIMESTAMP')))
