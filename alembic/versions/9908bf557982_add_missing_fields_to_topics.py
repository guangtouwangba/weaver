...
def upgrade() -> None:
    ...
    op.add_column('topics', sa.Column('status', sa.Enum('active', 'archived', 'draft', 'completed', name='topic_status_enum'), nullable=True, server_default='active'))
    op.add_column('topics', sa.Column('parent_topic_id', sa.BigInteger(), nullable=True))
    op.add_column('topics', sa.Column('settings', sa.JSON(), nullable=True, server_default='{}'))
    ...
    op.create_foreign_key('fk_topics_parent_topic_id', 'topics', 'topics', ['parent_topic_id'], ['id'], ondelete='SET NULL')
