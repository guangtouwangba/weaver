...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['file_id'], ['files.id'], name='fk_documents_file_id', ondelete='SET NULL')
    )
    ...
    op.execute('CREATE INDEX idx_documents_doc_metadata_gin ON documents USING gin (doc_metadata);')
    op.execute('CREATE INDEX idx_document_chunks_chunk_metadata_gin ON document_chunks USING gin (chunk_metadata);')
