...
def upgrade():
    ...
    op.add_column('topic_resources', 
                  sa.Column('file_hash', sa.VARCHAR(64), nullable=True))
    ...
    op.add_column('topic_resources',
                  sa.Column('access_level', sa.VARCHAR(20), nullable=False, server_default=sa.text("'private'")))
    ...
    op.create_unique_constraint('uq_topic_resources_file_hash', 
                               'topic_resources', ['file_hash'])
    
    # Create index for file_hash column for better query performance
    op.create_index('idx_topic_resources_file_hash', 
                   'topic_resources', ['file_hash'])
...
def downgrade():
    ...
    op.drop_constraint('uq_topic_resources_file_hash', 
                      'topic_resources', type_='unique')
