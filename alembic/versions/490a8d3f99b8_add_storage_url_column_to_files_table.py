...
def upgrade() -> None:
    ...
    op.add_column('files', sa.Column('storage_url', sa.String(length=2000), nullable=True))
    ...
    op.execute("COMMENT ON COLUMN files.id IS 'Primary key as UUID, mapped to String in application'")
    ...
    try:
        op.add_column('files', sa.Column('processing_status', sa.String(length=50), nullable=True))
    except Exception:
