...
def retrieve_documents(state: QueryState) -> QueryState:
    ...
    try:
        ...
        results = _VECTOR_INDEX.similarity_search_with_score(state.question, k=state.retriever_top_k)
    ...
    if state.document_ids:
        ...
        rag_documents = [doc for doc in rag_documents if doc.metadata.get("document_id") in state.document_ids]
