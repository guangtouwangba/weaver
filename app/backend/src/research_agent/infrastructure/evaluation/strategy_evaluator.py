...
from research_agent.infrastructure.evaluation.test_dataset import TestDataset, TestCase
from research_agent.infrastructure.evaluation.retrieval_metrics import RetrievalMetrics, RetrievalMetricsResult
...
class StrategyEvaluator:
    ...
    async def evaluate_strategy(
        ...
        for test_case in self.test_dataset.test_cases:
            try:
                ...
                logger.debug(f"Evaluated test case {test_case.id}: {len(retrieved_docs)} docs retrieved")
        ...
        if ragas_metrics_list:
            result.faithfulness = sum(m.get("faithfulness", 0) for m in ragas_metrics_list) / len(ragas_metrics_list)
            result.answer_relevancy = sum(m.get("answer_relevancy", 0) for m in ragas_metrics_list) / len(ragas_metrics_list)
            result.context_precision = sum(m.get("context_precision", 0) for m in ragas_metrics_list) / len(ragas_metrics_list)
        
        logger.info(f"Strategy {strategy_name} + {retrieval_mode} results: "
    ...
    def print_comparison(results: List[StrategyResult]):
        ...
        # Header
        print(f"{'Strategy':<20} {'Mode':<10} {'HR@5':<8} {'MRR':<8} {'P@5':<8} {'R@5':<8} "
        ...
        for r in sorted_results:
            print(f"{r.strategy_name:<20} {r.retrieval_mode:<10} "
        ...
        print(f"🏆 Best MRR: {best_mrr.strategy_name} + {best_mrr.retrieval_mode} ({best_mrr.mrr:.3f})")
        print(f"🏆 Best Faithfulness: {best_faith.strategy_name} + {best_faith.retrieval_mode} ({best_faith.faithfulness:.3f})")
        print()
    ...
    def export_to_csv(results: List[StrategyResult], file_path: str):
        ...
        with open(file_path, "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=[
                "strategy", "mode", "hit_rate", "mrr", "precision", "recall", "ndcg",
                "faithfulness", "answer_relevancy", "context_precision", "num_errors"
            ])
            writer.writeheader()
