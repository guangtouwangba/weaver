...
from research_agent.infrastructure.database.models import DocumentChunkModel, EntityModel, RelationModel
...
class GraphExtractorTask(BaseTask):
    ...
    async def execute(self, payload: Dict[str, Any], session: AsyncSession) -> None:
        ...
        for i in range(0, len(chunks), batch_size):
            ...
            try:
                ...
                logger.warning(f"Failed to extract from chunk batch {i}: {e}")
                continue
        
        logger.info(f"Extracted {len(graph.entities)} entities and {len(graph.relations)} relations")
    ...
    async def _save_graph(
        ...
        for relation in graph.relations:
            if relation.source_entity_id in entity_id_map and relation.target_entity_id in entity_id_map:
                model = RelationModel(
                    id=relation.id,
                    project_id=project_id,
        ...
        await session.flush()
        logger.info(f"Saved {len(graph.entities)} entities and {len(graph.relations)} relations to database")
