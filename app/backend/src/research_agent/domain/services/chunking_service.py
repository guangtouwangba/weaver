...
class NoChunkingStrategy(ChunkingStrategy):
    ...
    def chunk_text(self, text: str, metadata: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        return [{
            "content": text.strip(),
            "metadata": {**metadata, "chunk_type": "full_text"}
        }]
...
class RecursiveChunkingStrategy(ChunkingStrategy):
    ...
    def chunk_text(self, text: str, metadata: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            for chunk in chunks if len(chunk.strip()) >= self.config.min_chunk_size
        ]
...
class MarkdownChunkingStrategy(ChunkingStrategy):
    ...
    def chunk_text(self, text: str, metadata: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        return [
            {
                "content": chunk.strip(),
                "metadata": {**metadata, "chunk_type": "markdown"}
            }
            for chunk in chunks if len(chunk.strip()) >= self.config.min_chunk_size
        ]
...
class ChunkingService:
    ...
    def chunk_pages(
        ...
        logger.info(f"Created {len(chunks)} chunks from {len(pages)} pages using {strategy.__class__.__name__}")
        return chunks
