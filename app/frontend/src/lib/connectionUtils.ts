...
export function getBezierPath(
    ...
    let c1x = source.x, c1y = source.y;
    let c2x = target.x, c2y = target.y;

    switch (source.direction) {
        case 'E': c1x = source.x + offset; break;
        case 'W': c1x = source.x - offset; break;
        case 'N': c1y = source.y - offset; break;
        case 'S': c1y = source.y + offset; break;
    }

    switch (target.direction) {
        case 'E': c2x = target.x + offset; break;
        case 'W': c2x = target.x - offset; break;
        case 'N': c2y = target.y - offset; break;
        case 'S': c2y = target.y + offset; break;
    }
...
export function resolveAnchor(
    ...
    const anchor = anchorType === 'source' ? edge.sourceAnchor : edge.targetAnchor;

    if (!anchor || anchor === 'auto') {
        const [sourceAnchor, targetAnchor] = autoSelectAnchors(sourceNode, targetNode);
...
export function getOrthogonalPath(
    ...
    if ((source.direction === 'E' || source.direction === 'W') &&
    ...
    else if ((source.direction === 'N' || source.direction === 'S') &&
    ...
    else if ((source.direction === 'E' || source.direction === 'W') &&
        (target.direction === 'N' || target.direction === 'S')) {
