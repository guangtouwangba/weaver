...
const GridBackground = memo(
      ...
      <Group listening={false}>
        <Shape
          ...
          sceneFunc={(ctx) => {
            ...
            const endX =
              Math.ceil(
                (-viewport.x + dimensions.width) / viewport.scale / gridSize
              ) * gridSize;
            const endY =
              Math.ceil(
                (-viewport.y + dimensions.height) / viewport.scale / gridSize
              ) * gridSize;
  ...
  (prevProps, nextProps) => {
    ...
    const prevStartX =
      Math.floor(-prevProps.viewport.x / prevProps.viewport.scale / gridSize) *
      gridSize;
    const prevStartY =
      Math.floor(-prevProps.viewport.y / prevProps.viewport.scale / gridSize) *
      gridSize;
    const prevEndX =
      Math.ceil(
        (-prevProps.viewport.x + prevProps.dimensions.width) /
          prevProps.viewport.scale /
          gridSize
      ) * gridSize;
    const prevEndY =
      Math.ceil(
        (-prevProps.viewport.y + prevProps.dimensions.height) /
          prevProps.viewport.scale /
          gridSize
      ) * gridSize;

    const nextStartX =
      Math.floor(-nextProps.viewport.x / nextProps.viewport.scale / gridSize) *
      gridSize;
    const nextStartY =
      Math.floor(-nextProps.viewport.y / nextProps.viewport.scale / gridSize) *
      gridSize;
    const nextEndX =
      Math.ceil(
        (-nextProps.viewport.x + nextProps.dimensions.width) /
          nextProps.viewport.scale /
          gridSize
      ) * gridSize;
    const nextEndY =
      Math.ceil(
        (-nextProps.viewport.y + nextProps.dimensions.height) /
          nextProps.viewport.scale /
          gridSize
      ) * gridSize;
