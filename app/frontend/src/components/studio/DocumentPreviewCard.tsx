...
const PdfIcon = ({ size = 32, className }: { size?: number; className?: string }) => (
    <svg
        width={size}
        height={size}
        viewBox="0 0 24 24"
...
export default function DocumentPreviewCard({
    ...
    // Pass thumbnail URL in drag data ONLY (not shown in UI)
    const handleDragStart = (e: React.DragEvent) => {
        const dragData = {
            type: 'document',
            documentId: doc.id,
            title: doc.filename,
            thumbnailUrl: doc.thumbnail_url, // Keeps thumbnail for Canvas
    ...
    return (
        <div
            draggable={isPdf}
            onDragStart={isPdf ? handleDragStart : undefined}
            onClick={onSelect}
            className={`
