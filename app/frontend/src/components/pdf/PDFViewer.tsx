...
import { TextSelection, Highlight, HighlightColor } from './types';
...
export function PDFViewer({
  ...
  const handleColorSelect = useCallback(
    async (color: HighlightColor) => {
      ...
      try {
          ...
          setHighlights(prev => prev.filter(h => h.id !== tempId)); // Revert
          setErrorMessage('Failed to create highlight');
      }
    },
    [textSelection, toolbarPosition, documentId, onHighlightCreate, isCreatingHighlight]
  );
  ...
  const handleDeleteHighlight = useCallback(async () => {
    ...
    try {
        ...
        setHighlights(prev => [...prev, deleted]);
  ...
  const handleSaveEditNote = useCallback(async () => {
      ...
      setHighlights(prev => prev.map(h => h.id === original.id ? updated : h));
      setNoteDialogOpen(false);
      setNoteText('');
      setSelectedHighlight(null);
      ...
      try {
          ...
          setHighlights(prev => prev.map(h => h.id === original.id ? original : h));
