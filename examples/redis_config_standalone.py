...
def example_cluster_config():
    ...
    print(f"需要全覆盖: {'是' if cluster_config.cluster_require_full_coverage else '否'}")
...
def example_environment_configs():
    ...
    print("环境配置对比:")
    print(f"  开发: {dev_config.host} (连接: {dev_config.max_connections}, TTL: {dev_config.default_ttl}s)")
    print(f"  测试: {test_config.host} (连接: {test_config.max_connections}, TTL: {test_config.default_ttl}s)")
    print(f"  生产: {prod_config.host} (连接: {prod_config.max_connections}, TTL: {prod_config.default_ttl}s, SSL: {prod_config.ssl})")
