...
class WorkerConfig(BaseModel):
    ...
    memory_limit: int = Field(
        default=1024 * 1024 * 1024, description="内存限制（字节）"
    )
...
class TaskConfig(BaseModel):
    ...
    store_errors_even_if_ignored: bool = Field(
        default=True, description="即使忽略也存储错误"
    )
    serializer: str = Field(default="json", description="序列化器")
...
class RetryConfig(BaseModel):
    ...
    autoretry_for: List[str] = Field(
        default_factory=list, description="自动重试的异常类型"
    )
...
class CeleryConfig(BaseModel):
    ...
    worker_max_tasks_per_child: int = Field(
        default=1000, description="每个子进程最大任务数"
    )
    ...
    task_annotations: Dict[str, Dict[str, Any]] = Field(
        default_factory=lambda: {"*": {"max_retries": 3, "default_retry_delay": 60}},
        description="任务注解",
    )

    # 监控设置
    worker_send_task_events: bool = Field(
        default=True, description="工作进程发送任务事件"
    )
    ...
    task_routes: Dict[str, Dict[str, str]] = Field(
        default_factory=dict, description="任务路由"
    )
...
class AnthropicConfig(BaseModel):
    ...
    chat_model: str = Field(
        default="claude-3-sonnet-20240229", description="Default chat model"
    )
...
class HuggingFaceConfig(BaseModel):
    ...
    # Model configurations
    chat_model: str = Field(
        default="microsoft/DialoGPT-medium", description="Default chat model"
    )
    embedding_model: str = Field(
...
class LocalLLMConfig(BaseModel):
    ...
    api_base: str = Field(
        default="http://localhost:11434", description="Local LLM API Base URL"
    )
...
class EmbeddingConfig(BaseModel):
    ...
    provider: str = Field(
        default="openai", description="Embedding provider (openai, huggingface, local)"
    )
    ...
    batch_size: int = Field(
        default=100, description="Batch size for embedding requests"
    )
...
class ChatConfig(BaseModel):
    ...
    provider: str = Field(
        default="openai",
        description="Chat provider (openai, anthropic, huggingface, local)",
    )
...
class AIConfig(BaseModel):
    ...
    embedding: EmbeddingConfig = Field(
        default_factory=EmbeddingConfig, description="Embedding service configuration"
    )
    chat: ChatConfig = Field(
        default_factory=ChatConfig, description="Chat service configuration"
    )
    ...
    rate_limit_tokens_per_minute: int = Field(
        default=100000, description="Token rate limit per minute"
    )
...
class AppConfig(BaseSettings):
    ...
    openapi_url: Optional[str] = Field(
        default="/openapi.json", description="OpenAPI JSON URL"
    )
    ...
    def get_logging_config(self) -> Dict[str, Any]:
        ...
        if log_file:
            config["handlers"]["file"] = {
                ...
                "backupCount": 30,
                "formatter": (
                    "json" if self.environment == Environment.PRODUCTION else "default"
                ),
                "level": log_level,
            }
