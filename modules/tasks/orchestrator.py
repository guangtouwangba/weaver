...
class TaskOrchestrator:
            ...
            while len(executed_tasks) < len(workflow.tasks):
                ...
                for task_def in workflow.tasks:
                    ...
                    dependencies_met = all(
                        dep in executed_tasks and 
                        execution.tasks[dep].status == TaskStatus.SUCCESS
                ...
                if not ready_tasks:
                    # 没有可执行的任务，检查是否有失败的任务
                    failed_tasks = [
                        task_name for task_name, task_exec in execution.tasks.items()
    ...
    def _prepare_task_kwargs(self, execution: WorkflowExecution, task_def: TaskDefinition) -> Dict[str, Any]:
        """准备任务参数"""
        # 基础参数从上下文中获取
