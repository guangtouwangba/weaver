    ...
    try:
        ...
        try:
            ...
            document.metadata.update({
                ...
                "original_type": content_type.value if hasattr(content_type, 'value') else str(content_type),
                "detected_type": ContentType.TXT.value
            })
            return document
        except Exception as fallback_error:
            raise Exception(f"Document loading failed, unable to find suitable loader: {e}, text loader fallback also failed: {fallback_error}")
...
class FileUploadCompleteHandler(ITaskHandler):
    ...
    async def handle(self,
        ...
        """
        try:
            logger.info(f"Processing file upload completion: {file_id}, file storage key: {file_path}")
    ...
    async def _process_document_with_rag(self, 
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Document loaded successfully: {document.title}, content length: {len(document.content)}")
                ...
                logger.info(f"RAG processing completed: {file_id}, request_id: {rag_result.get('rag_request_id')}")
        ...
        async with db.get_session() as session:
            try:
                ...
                try:
                    ...
                    logger.warning(f"RAG pipeline initialization failed, will use traditional processing: {e}")
                ...
                # Create document and trigger RAG processing
                ...
                raise Exception(f"Document creation and RAG processing failed: {e}")
...
class TempFileCleanupHandler(ITaskHandler):
    ...
    async def handle(self,
        ...
        try:
            ...
            logger.info(f"Temporary file cleanup completed: deleted {len(deleted_files)}, failed {len(failed_files)}, skipped {len(skipped_files)}")
            return result
