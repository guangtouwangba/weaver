...
from ..database.models import Topic as TopicModel, File as FileModel, Document as DocumentModel
from .topic import TopicSchema, TopicResponse
from .file import FileSchema, FileResponse
from .document import DocumentSchema, DocumentResponse
...
def topic_to_schema(topic_model: TopicModel, include_stats: bool = False) -> TopicSchema:
    ...
    if include_stats:
        ...
        schema_data.update({
            'file_count': len(topic_model.files) if hasattr(topic_model, 'files') and topic_model.files else 0,
...
def topic_to_response(topic_model: TopicModel, file_count: int = 0, document_count: int = 0) -> TopicResponse:
...
def file_to_schema(file_model: FileModel, include_topic_name: bool = False) -> FileSchema:
    ...
    if include_topic_name:
        ...
        topic_name = file_model.topic.name if hasattr(file_model, 'topic') and file_model.topic else None
...
# Document转换器
def document_to_schema(document_model: DocumentModel, include_chunk_count: bool = False) -> DocumentSchema:
    ...
    if include_chunk_count:
        ...
        chunk_count = len(document_model.chunks) if hasattr(document_model, 'chunks') and document_model.chunks else 0
