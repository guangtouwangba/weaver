...
class ProcessingResult(BaseSchema):
    ...
    chunks_created: int = Field(ge=0, description="创建的文档块数量")
...
class SearchResult(BaseSchema):
    ...
    document_id: str = Field(description="文档ID")
    chunk_id: Optional[str] = Field(default=None, description="文档块ID")
    title: str = Field(description="文档标题")
    content: str = Field(description="匹配内容")
    score: float = Field(ge=0.0, le=1.0, description="相关性得分")
    content_type: str = Field(description="内容类型")
    file_id: Optional[str] = Field(default=None, description="来源文件ID")
...
class SearchResponse(BaseSchema):
    ...
    search_time: float = Field(ge=0.0, description="搜索耗时（秒）")
