...
class ProcessingRequest(BaseSchema):
    ...
    metadata: Optional[Dict[str, Any]] = Field(default_factory=dict, description="处理元数据")
...
class SearchRequest(BaseSchema):
    ...
    query: str = Field(
        min_length=1, description="搜索查询", example="机器学习中的决策树算法原理是什么？"
    )
    search_type: SearchType = Field(default=SearchType.SEMANTIC, description="搜索类型")
    limit: int = Field(default=10, ge=1, le=100, description="返回结果数量限制", example=5)
    content_type: Optional[ContentType] = Field(default=None, description="内容类型过滤")
...
class UploadUrlRequest(BaseSchema):
    ...
    file_size: Optional[int] = Field(default=None, ge=0, description="文件大小", example=2048576)
...
class AddResourceRequest(BaseSchema):
    ...
    resource_type: Optional[str] = Field(default="file", description="资源类型", example="file")
    title: Optional[str] = Field(
        default=None, description="资源标题", example="机器学习算法详解.pdf"
    )
    description: Optional[str] = Field(
        default=None, description="资源描述", example="关于机器学习核心算法的详细技术文档"
    )
    ...
    priority: Optional[int] = Field(default=0, ge=0, le=10, description="优先级 (0-10)", example=5)
