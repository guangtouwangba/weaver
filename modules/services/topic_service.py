...
from ..schemas import (
    ...
    AddResourceRequest, FileResponse,
    topic_to_response, topics_to_responses, file_to_response
)
...
class TopicService(BaseService):
    ...
    async def list_topics(self, 
                         ...
                         category: Optional[str] = None) -> TopicList:
        """获取主题列表"""
        try:
            # 构建过滤条件
            filters = {}
    ...
    async def add_resource_to_topic(self, topic_id: int, resource_data: AddResourceRequest) -> FileResponse:
        """向主题添加资源（文件）"""
        try:
            # 验证主题是否存在
            topic = await self.topic_repo.get_topic_by_id(topic_id)
            ...
            if file_record.topic_id and file_record.topic_id != topic_id:
                self.logger.warning(f"文件 {resource_data.file_id} 已关联到主题 {file_record.topic_id}，将更新为新主题 {topic_id}")
    ...
    async def get_topic_resources(self, topic_id: int, 
                                 page: int = 1, 
