...
from modules.repository import FileRepository, TopicRepository
...
from modules.storage import IStorage
        ...
        try:
            ...
            if request.actual_size:
                await self.file_repo.update(
                    request.file_id,
                    {"file_size": request.actual_size, "file_hash": request.file_hash},
                )
    ...
    async def get_file_download_url(self, file_id: str) -> Optional[str]:
        ...
        try:
            ...
            download_url = await self.storage.generate_presigned_url(
                key=file_record.storage_key, expires_in=3600, method="GET"  # 1小时
            )
