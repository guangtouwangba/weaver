...
class RAGProcessorConfig:
    ...
    def __init__(
        ...
        enable_summary_generation: bool = True,  # 新增：启用摘要生成
        summary_min_content_length: int = 500,  # 新增：摘要生成最小内容长度
    ...
    async def process_document(
        ...
        try:
            ...
            if (self.config.enable_summary_generation and 
                ...
                logger.info(f"开始摘要生成阶段")
                summary_result = await self._generate_document_summary(
                    document, topic_id, file_id
                )
                if summary_result:
                    logger.info(f"摘要生成完成: {summary_result.id}")
                else:
                    logger.warning("摘要生成失败，但不影响整体处理")
            else:
                logger.info(f"跳过摘要生成: 内容长度 {len(document.content)} < {self.config.summary_min_content_length}")
    ...
    async def _generate_document_summary(
        ...
        try:
            ...
            async with get_session() as session:
                ...
                logger.info(f"✅ 摘要生成完成: {document.id}")
                
                return summary_doc
            
        except Exception as e:
            logger.error(f"❌ 文档摘要生成失败 {document.id}: {e}")
            # 摘要生成失败不应该影响整个RAG流程
            return None
