...
class ChunkingProcessor(IDocumentProcessor):
        ...
        try:
            ...
            if strategy == ChunkingStrategy.SEMANTIC:
                ...
                chunks = await self.text_processor.create_chunks(
                    document, strategy=strategy, chunk_size=chunk_size, overlap=overlap
                )
    ...
    async def process_document(self, request: ProcessingRequest) -> ProcessingResult:
        ...
        try:
            ...
            logger.info(
                f"高级文档处理完成 {document.id}: {len(chunks)} 块, {processing_time:.2f}ms"
            )
            return result
    ...
    async def _optimize_parameters(
        self, request: ProcessingRequest, analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
        """根据文档分析优化参数"""
        params = {
    ...
    async def _create_semantic_chunks(
        ...
        for paragraph in paragraphs:
            ...
            # 如果相似度高且大小允许，合并到当前块
            if similarity > 0.3 and len(current_chunk + paragraph) <= chunk_size:
                if current_chunk:
                    current_chunk += "\n\n" + paragraph
                else:
    ...
    async def _calculate_quality_metrics(
        self, chunks: List[DocumentChunk], analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
        """计算质量指标"""
        if not chunks:
