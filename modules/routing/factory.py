...
class RoutingEngineFactory:
    ...
    async def create_default_engine(
        ...
        await RoutingEngineFactory._register_strategies(
            engine, config_loader, openai_client, semantic_config
        )
        ...
        # 配置引擎设置 - 默认策略由调用者设置
        # engine.set_default_strategy("configurable_keyword")  # 让调用者决定默认策略
    ...
    async def create_llm_first_engine(
        ...
        if not openai_client and not OPENAI_AVAILABLE:
            ...
            return await RoutingEngineFactory.create_default_engine(
                chat_service, config_dir=config_dir, semantic_config=semantic_config
            )
    ...
    async def create_keyword_only_engine(
        chat_service: Optional[Any] = None, config_dir: str = "config/routing"
    ) -> QueryRoutingEngine:
        """
    ...
    async def _register_strategies(
        ...
        if (
            ...
            and semantic_config
            and semantic_config.get("enabled", False)
        ):
            try:
                # 加载语义路由配置
                ...
                # 创建编码器配置
                encoder_config = RoutingEngineFactory._build_encoder_config(
                    semantic_config
                )
                ...
                logger.info(
                    f"Semantic Router策略已注册，编码器: {semantic_config.get('encoder_type', 'fastembed')}"
                )
            except Exception as e:
    ...
    async def _register_handlers(
        engine: QueryRoutingEngine, chat_service: Optional[Any] = None
    ) -> None:
        """注册所有查询处理器"""
    ...
    async def _add_default_rules(engine: QueryRoutingEngine) -> None:
        ...
        def is_admin_command(query: str, context: Dict[str, Any]) -> bool:
            return query.startswith("/admin") and context.get("user_role") == "admin"
    ...
    @staticmethod
    async def _load_semantic_routes_config(
    ...
    def _build_encoder_config(semantic_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if encoder_type == "openai":
            # OpenAIEncoder 使用环境变量 OPENAI_API_KEY，不接受 api_key 参数
            import os

            if semantic_config.get("openai_api_key"):
                os.environ["OPENAI_API_KEY"] = semantic_config["openai_api_key"]
            # OpenAIEncoder 使用 name 参数而不是 model 参数
            if semantic_config.get("encoder_model"):
                encoder_config["name"] = semantic_config["encoder_model"]

        elif encoder_type == "cohere":
            if semantic_config.get("cohere_api_key"):
                encoder_config["cohere_api_key"] = semantic_config["cohere_api_key"]
            # CohereEncoder 也使用 name 参数而不是 model 参数
            if semantic_config.get("encoder_model"):
                encoder_config["name"] = semantic_config["encoder_model"]

        elif encoder_type == "huggingface":
            encoder_config["model"] = semantic_config.get(
    ...
    async def create_semantic_router_engine(
        ...
        if not SEMANTIC_ROUTER_AVAILABLE:
            ...
            return await RoutingEngineFactory.create_default_engine(
                chat_service, config_dir=config_dir
            )
        ...
        if semantic_config and semantic_config.get("enabled", True):
            await RoutingEngineFactory._register_strategies(
                engine, config_loader, semantic_config=semantic_config
            )
