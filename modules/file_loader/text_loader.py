...
# Optional dependency: chardet for better encoding detection
try:
    import chardet
    HAS_CHARDET = True
except ImportError:
...
class TextFileLoader(IFileLoader):
    ...
    def __init__(self, 
        ...
        logger.info(f"TextFileLoader initialized: default_encoding={default_encoding}, max_file_size={max_file_size/1024/1024:.1f}MB")
    ...
    async def load_file(self, file_path: str, metadata: Optional[Dict[str, Any]] = None) -> Document:
        ...
        try:
            ...
            logger.info(f"Successfully loaded text file: {file_path} ({len(content)} characters, encoding: {encoding})")
            return document
    ...
    async def _detect_encoding(self, file_path: str) -> str:
        ...
        try:
                ...
                with open(file_path, 'rb') as f:
                    ...
                    if sample:
                        ...
                        if detection and detection.get('confidence', 0) > 0.7:
                            ...
                            logger.debug(f"Detected encoding: {detected_encoding} (confidence: {detection.get('confidence', 0):.2f})")
            ...
            logger.warning(f"Cannot detect encoding, using default encoding: {self.default_encoding}")
    ...
    async def _read_file_content(self, file_path: str, encoding: str) -> str:
        ...
        try:
            ...
            logger.warning(f"Failed to read with encoding {encoding}, trying fallback encodings: {e}")
            ...
            for fallback_encoding in self.fallback_encodings:
                try:
                    ...
                    logger.info(f"Successfully read file using fallback encoding {fallback_encoding}")
                    return content
                    
                except Exception as fallback_error:
                    logger.debug(f"Fallback encoding {fallback_encoding} also failed: {fallback_error}")
                    continue
