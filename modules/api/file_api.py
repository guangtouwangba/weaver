...
async def generate_upload_url(
    request: UploadUrlRequest, service: FileService = Depends(get_file_service)
):
    """
    # 生成安全的文件上传签名URL
...
async def get_file(file_id: str, service: FileService = Depends(get_file_service)):
    ...
    try:
        async with service:
            ...
            return APIResponse(success=True, data=file_record)
...
async def update_file(
    ...
    try:
        async with service:
            ...
            return APIResponse(success=True, message="文件更新成功", data=file_record)
    except HTTPException:
        raise
    except Exception as e:
...
async def delete_file(file_id: str, service: FileService = Depends(get_file_service)):
    ...
    try:
        async with service:
            ...
            return APIResponse(success=True, message="文件删除成功")
    except HTTPException:
        raise
    except Exception as e:
...
async def get_file_download_url(
    ...
    try:
        async with service:
            ...
            return APIResponse(success=True, data={"download_url": download_url})
    except HTTPException:
        raise
    except Exception as e:
