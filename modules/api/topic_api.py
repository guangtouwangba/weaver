...
async def get_file_service_for_topic(
    session: AsyncSession = Depends(get_db_session),
) -> FileService:
    """获取文件服务实例（用于主题相关操作）"""
...
async def create_topic(
    ...
    try:
        ...
        return APIResponse(success=True, message="主题创建成功", data=topic)
...
async def update_topic(
    ...
    try:
        ...
        return APIResponse(success=True, message="主题更新成功", data=topic)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except HTTPException:
        raise
...
async def delete_topic(
    ...
    try:
        ...
        return APIResponse(success=True, message="主题删除成功")
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except HTTPException:
        raise
...
async def get_topic_files(
    ...
    try:
        ...
        return APIResponse(
            success=True, data=file_list, message=f"获取主题{topic_id}的文件列表成功"
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
...
async def add_resource_to_topic(
    ...
    try:
        ...
        return APIResponse(
            success=True, message=f"成功将资源添加到主题 {topic_id}", data=file_response
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
