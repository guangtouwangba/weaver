...
from modules.orchestrator.base import IOrchestrator, OrchestrationError
...
from modules.processors.base import IDocumentProcessor
...
class DocumentOrchestrator(IOrchestrator):
    ...
    async def initialize_rag_pipeline(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"RAG管道初始化完成: {self.embedding_provider} + {self.vector_store_provider}"
            )
                ...
                result = OrchestrationResult(
                    ...
                    steps_completed=[
                        {"step": "file_loading", "status": "completed", "time_ms": 0},
                        {
                            ...
                            "status": "completed",
                            "time_ms": processing_result.processing_time_ms,
                        },
                        {
                            "step": "storage",
                            "status": (
                        ...
                        {
                            "step": "indexing",
                            "status": "completed" if self.search_backend else "skipped",
                            "time_ms": 0,
                        },
                    ],
                    metadata={
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health_status["file_loader"] = {"status": "error", "error": str(e)}
        ...
        try:
            ...
            health_status["document_processor"] = {"status": "error", "error": str(e)}
        ...
        if self.storage_backend:
            try:
                ...
                health_status["storage_backend"] = {"status": "error", "error": str(e)}
        else:
        ...
        if self.search_backend:
            try:
                ...
                health_status["search_backend"] = {"status": "error", "error": str(e)}
        else:
    ...
    async def _load_file(self, request: OrchestrationRequest) -> Document:
        ...
        try:
            ...
            file_request = FileLoadRequest(
                file_path=request.source_path, metadata=request.metadata or {}
            )
    ...
    async def _process_with_rag_pipeline(
        self, request: OrchestrationRequest, operation_id: str, start_time: float
    ) -> OrchestrationResult:
        """使用RAG管道处理文档"""
        try:
            # 创建RAG管道处理请求
