...
class FileUploadService(IFileUploadService):
    ...
    async def generate_upload_url(
        ...
        try:
            ...
            upload_info = await self.storage.generate_signed_upload_url(
                file_key=file_key, content_type=content_type, expires_in=expires_in
            )
    ...
    async def confirm_upload(
        ...
        try:
            ...
            async with get_session() as session:
                ...
                if not file_info:
                    return {"success": False, "error": f"文件记录不存在: {file_id}"}

                # 验证文件是否真的存在于存储中
                file_exists = await self.storage.file_exists(file_info.storage_key)
                if not file_exists:
                    return {"success": False, "error": "文件未找到，上传可能失败"}
    ...
    async def _submit_rag_task_async(
        self, file_id: str, file_path: str, content_type: str, topic_id: Optional[int]
    ) -> None:
        """
        ...
        try:
            ...
            async with get_session() as session:
                ...
                download_url = await self.storage.generate_signed_download_url(
                    file_key=file_info.storage_key, expires_in=expires_in
                )
        ...
        try:
            ...
            async with get_session() as session:
                ...
                if success:
                    ...
                    logger.warning(
                        f"删除文件部分失败: {file_id} (存储: {storage_deleted}, 数据库: {db_deleted})"
                    )
            ...
            async with get_session() as session:
                ...
                if topic_id:
                    return await file_repo.list_topic_files(
                        topic_id=topic_id, page=page, page_size=page_size, status=status
                    )
                else:
                    # 这里可以扩展为列出所有文件
                    return {"files": [], "message": "列出全部文件功能待实现"}
